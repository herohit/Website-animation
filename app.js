...
function animateSlides(){
    ...
    sliders.forEach((slide,index,slides) =>{
        ...
        // slideTl.fromTo(nav,{y:'-100%'},{y:'10%'},'-=0.5');

        //animate when we scroll by adding scene
        ...
        pageScene=new ScrollMagic.Scene({
            triggerElement:slide,
            duration:"100%", //whole height of the slide
            triggerHook:0
    ...
    transitions:[
        {
            leave({current,next}){
                ...
                tl.fromTo('.swipe',0.75,{x:'-100%'},{c:'0%',onComplete:done},'-=0.5');
            },
            enter({current,next}){
                let done = this.async();
                //scroll to top
...
function detailAnimation(){
    ...
    slides.forEach((slide,index,slides)=>{
        ...
        slideTl.fromTo(nextImg,{x:'60%',y:'50%',opacity:0},{x:'0%',y:'0%',opacity:1})
